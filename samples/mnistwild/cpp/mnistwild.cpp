...
MnistWildApp::~MnistWildApp() {
  if (croppedImageReadback_) {
    croppedImageReadback_->Stop();
  }
  keepRunning = false;
  initCv.notify_all();
...
void MnistWildApp::CreateReadback() {
  if (!frameworkSession || !croppedImageGlobal) {
    Log::Write(Log::Level::Error, "CreateReadback skipped: framework session or cropped image tensor missing");
    return;
  }
  if (croppedImageReadback_) {
    return;
  ...
  Log::Write(Log::Level::Info, "Initializing TensorReadback for vst_output_left_u8 target");

  TensorReadback::Target target{
      .tensor = croppedImageGlobal,
  ...
  croppedImageReadback_ = std::make_unique<TensorReadback>(frameworkSession, std::vector<TensorReadback::Target>{std::move(target)}, config);
  croppedImageReadback_->Start();
}

void MnistWildApp::HandleReadbackResult(TensorReadbackResult&& result) {
  if (result.data.empty()) {
    Log::Write(Log::Level::Info,
               Fmt("TensorReadback for %.*s returned empty buffer",
  ...
  if (!result.dimensions.empty()) {
    dims = std::to_string(result.dimensions[0]);
    for (std::size_t i = 1; i < result.dimensions.size(); ++i) {
      dims.append("x").append(std::to_string(result.dimensions[i]));
    }
  }

  Log::Write(Log::Level::Info,
             Fmt("TensorReadback for %.*s: dataSize=%zu bytes (dims=%s, channels=%d, dataType=%d)",
                 static_cast<int>(result.name.size()), result.name.data(),
                 result.data.size(), dims.c_str(), result.channels,
                 static_cast<int>(result.dataType)));
}
...
void MnistWildApp::RequestPermission(struct android_app* app) {
  Log::Write(Log::Level::Info, "Requesting camera permission from activity");
  if (app == nullptr || app->activity == nullptr || app->activity->vm == nullptr) {
    Log::Write(Log::Level::Error, "Invalid android_app passed to RequestPermission");
    return;
  }
  ...
  if (env == nullptr) {
    Log::Write(Log::Level::Error, "Failed to attach JNI environment while requesting permission");
    return;
  }
